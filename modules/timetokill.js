import { summAll } from "./calculate.js";
import { resultWithArmor } from "./calculate.js";
...
export default function TTK(params) {
  ...
  let скорострельность = document.querySelector(
  ...
  let приведенкаСбронебойностью =
    ((100 + (py - (py / 100) * бронебойность)) / 100) * (100 + hp);
  ...
  if (бронебойность == 0) {
    ...
    document.querySelector("privedenkanumber").innerHTML =
      приведенкаСбронебойностью.toFixed();
  }
  ...
  document.querySelector(".damageToBody").innerHTML = `
    ...
    <div class="damageToBodylabel">Количество попаданий = ${(
      приведенкаСбронебойностью / урон
    ).toFixed()}</div>
    <div class="damageToBodylabel">ТТК = ${(
      округл * времяМеждуВыстрелами
    ).toFixed(1)} сек</div>
    `;
