...
export default function sborka() {
    let sborkaObject = [{
            ...
            'value': artefacts[`${document.querySelector('#firstArtSlot .art').innerHTML}`],
            'lvl': document.querySelector('#firstArtSlot .lvl').value,
            'quality': document.querySelector('#firstArtSlot .quality').value,
            'firstDop': document.querySelector('#firstDopSlot .dops').childNodes[1].getAttribute('value'),
            'secondDop': document.querySelector('#firstDopSlot .dops').childNodes[5].getAttribute('value'),
            'thirdDop': document.querySelector('#firstDopSlot .dops').childNodes[9].getAttribute('value'),
        }, {
            'name': document.querySelector('#secondArtSlot .art').innerHTML,
            'value': artefacts[`${document.querySelector('#secondArtSlot .art').innerHTML}`],
            'lvl': document.querySelector('#secondArtSlot .lvl').value,
            'quality': document.querySelector('#secondArtSlot .quality').value,
            'firstDop': document.querySelector('#secondDopSlot .dops').childNodes[1].getAttribute('value'),
            'secondDop': document.querySelector('#secondDopSlot .dops').childNodes[5].getAttribute('value'),
            'thirdDop': document.querySelector('#secondDopSlot .dops').childNodes[9].getAttribute('value'),
        },
        {
            'name': document.querySelector('#thirdArtSlot .art').innerHTML,
            'value': artefacts[`${document.querySelector('#thirdArtSlot .art').innerHTML}`],
            'lvl': document.querySelector('#thirdArtSlot .lvl').value,
            'quality': document.querySelector('#thirdArtSlot .quality').value,
            'firstDop': document.querySelector('#thirdDopSlot .dops').childNodes[1].getAttribute('value'),
            'secondDop': document.querySelector('#thirdDopSlot .dops').childNodes[5].getAttribute('value'),
            'thirdDop': document.querySelector('#thirdDopSlot .dops').childNodes[9].getAttribute('value'),
        },
        {
            'name': document.querySelector('#fourArtSlot .art').innerHTML,
            'value': artefacts[`${document.querySelector('#fourArtSlot .art').innerHTML}`],
            'lvl': document.querySelector('#fourArtSlot .lvl').value,
            'quality': document.querySelector('#fourArtSlot .quality').value,
            'firstDop': document.querySelector('#fourDopSlot .dops').childNodes[1].getAttribute('value'),
            'secondDop': document.querySelector('#fourDopSlot .dops').childNodes[5].getAttribute('value'),
            'thirdDop': document.querySelector('#fourDopSlot .dops').childNodes[9].getAttribute('value'),
        },
        {
            'name': document.querySelector('#fiveArtSlot .art').innerHTML,
            'value': artefacts[`${document.querySelector('#fiveArtSlot .art').innerHTML}`],
            'lvl': document.querySelector('#fiveArtSlot .lvl').value,
            'quality': document.querySelector('#fiveArtSlot .quality').value,
            'firstDop': document.querySelector('#fiveDopSlot .dops').childNodes[1].getAttribute('value'),
            'secondDop': document.querySelector('#fiveDopSlot .dops').childNodes[5].getAttribute('value'),
            'thirdDop': document.querySelector('#fiveDopSlot .dops').childNodes[9].getAttribute('value'),
        },
        {
            'name': document.querySelector('#sixArtSlot .art').innerHTML,
            'value': artefacts[`${document.querySelector('#sixArtSlot .art').innerHTML}`],
            'lvl': document.querySelector('#sixArtSlot .lvl').value,
            'quality': document.querySelector('#sixArtSlot .quality').value,
            'firstDop': document.querySelector('#sixDopSlot .dops').childNodes[1].getAttribute('value'),
            'secondDop': document.querySelector('#sixDopSlot .dops').childNodes[5].getAttribute('value'),
            'thirdDop': document.querySelector('#sixDopSlot .dops').childNodes[9].getAttribute('value'),
        },
    ]
            ...
            for (const key in value) {
                ...
                if (key != 'Доп') {
                    if (quality <= 100) {
                        if (lvl > 0) {
                            if (max > 0) {
                                if (key != 'Радиация' && key != 'Пси-излучение' && key != 'Биозаражение' && key != 'Кровотечение' && key != 'Температура') {
                                    result.push({
                                        [key]: (min + ((max - min) / 100) * quality) + ((min + ((max - min) / 100) * quality) / 100) * (lvl * 2)
                                    })
                                } else {
                                    result.push({
                        ...
                        if (lvl > 0) {
                            if (max > 0) {
                                if (key != 'Радиация' && key != 'Пси-излучение' && key != 'Биозаражение' && key != 'Кровотечение' && key != 'Температура') {
                                    ...
                                    result.push({
                                        [key]: mind + (((max - mind) / 100) * ((quality - (100 + 10 * rare)) * 10))
                                    })
                                }
                            } else {
                                if (key != 'Радиация' && key != 'Пси-излучение' && key != 'Биозаражение' && key != 'Кровотечение' && key != 'Температура') {
...
function pushDops(sborkaObject) {
    ...
    for (const iterator of sborkaObject) {
        ...
        if (iterator.name != 'Выберите артефакт') {
            ...
            if (iterator.secondDop != null && iterator.secondDop != iterator.firstDop) {
                appendIterator(artefacts[iterator.name]['Доп'], iterator.secondDop)
            }
            if (iterator.thirdDop != null && iterator.thirdDop != iterator.secondDop) {
...
function pushArmor() {
    if (document.querySelector('.armor').innerHTML != 'Выберите броню') {
        ...
        for (const key in armor[name]) {
            ...
            if (key == 'Защита от разрыва' || key == 'Защита от взрыва' || key == 'Защита от хим.ожога' || key == 'Защита от электричества' || key == 'Защита от ожога' || key == 'Пулестойкость') {
    ...
    if (document.querySelector('.container').innerHTML != 'Выберите контейнер') {
        ...
        for (const key in summAll) {
            if (Object.hasOwnProperty.call(summAll, key)) {
                ...
                if (key == 'Радиация' || key == 'Температура' || key == 'Биозаражение' || key == 'Пси-излучение') {
                    if (summAll[key] > 0) {
